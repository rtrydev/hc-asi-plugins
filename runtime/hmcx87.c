/* HMC Reduced-Precision x87 — runtime patch loader (32-bit ASI).
 *
 * Loads .x87 patch files (produced by tools/translate.py) from the
 * "<asi dir>/hmc_reduced_x87/" directory. When the matching game module is
 ...
 *
 * For Contracts the target module is HitmanContracts.exe itself: the game is
 * monolithic (the RenderD3D renderer, sound and SDL are statically linked
 * into the exe), and the on-disk image is NOT packed, so its x87 — the
 * software vertex-transform/lighting path plus the AI/animation/physics
 * game-logic math — is translated straight from the executable, with no
 * memory-dump step (unlike Hitman 2's packed exe). The loader is
 * module-agnostic: it applies whatever <module>.x87 files are present, keyed
 * by the module name in the header.
 */
#include <windows.h>
#include <stdio.h>
#include <stdint.h>
 ...
 * Layout must match tools/x87/codegen.py. 16-byte aligned. */
typedef struct {
    double zero;        /* 0x00 */
    double one;         /* 0x08 */
    double pi;          /* 0x10 */
...
/* ---- diagnostics (used by HMC_DIAG builds of helpers.S) ---- */
...
static int apply_patch(Pending *p, HMODULE mod)
{
    ...
    if (memcmp(h->magic, "HMCONX87", 8) || h->version != 1) {
    ...
    /* optional trailing section: 5 original entry bytes per function */
    uint8_t *orig = blob_src + h->blob_total;
    int has_orig = (uint32_t)(orig - buf) + h->n_funcs * 5 <= size;
    ...
    /* Entry-byte gate: verify the live module's function entries match the
     * originals the blob was built from before hooking. HitmanContracts.exe is
     * NOT packed, so this passes on the first poll (the mapped image equals the
     * on-disk image the translator read). The gate is retained as a safety net:
     * it also covers the general packed case (a module whose .text is still
     * garbage returns "not ready" so the poller retries) at no cost here.
     *
     * Only gate at delta==0 (module at its preferred base, so no base
     * relocations rewrote the entry bytes). A relocated module's entry bytes
     * legitimately differ from the on-disk originals wherever a reloc lands in
     * the first 5 bytes, so the byte match can't be used there. */
    int gate = has_orig && delta == 0;
    if (gate) {
        uint32_t match = 0;
        for (uint32_t i = 0; i < h->n_funcs; i++)
            if (!memcmp(base + funcs[i].rva, orig + i * 5, 5)) match++;
        if (match * 100 < h->n_funcs * 90) {
            HeapFree(GetProcessHeap(), 0, buf);
            return -1;   /* not unpacked yet — retry later */
        }
    }
    ...
    /* install entry hooks (skip any func whose live bytes don't match the
     * recorded original — protects against a partially-unpacked or modified
     * entry) */
    unsigned installed = 0, skipped = 0;
    for (uint32_t i = 0; i < h->n_funcs; i++) {
        uint8_t *entry = base + funcs[i].rva;
        uint8_t *target = blob + funcs[i].blob_off;
        if (gate && memcmp(entry, orig + i * 5, 5)) { skipped++; continue; }
        DWORD old;
    ...
    logf_("[%s] applied: %u/%lu hooks (%u skipped), blob %lu KB at %p "
          "(delta %+d)", p->module, installed, (unsigned long)h->n_funcs,
          skipped, (unsigned long)h->blob_total / 1024, blob, delta);
    HeapFree(GetProcessHeap(), 0, buf);
    return 1;
fail:
    HeapFree(GetProcessHeap(), 0, buf);
...
static void check_pending(void)
{
    ...
    for (int i = 0; i < g_n_pending; i++) {
        ...
        if (m) {
            /* -1 == module present but not unpacked yet: leave pending so the
             * poller retries. 0/1 == permanent (failed / applied). */
            if (apply_patch(&g_pending[i], m) != -1)
                g_pending[i].applied = 1;
        }
    }
...
static void init(HMODULE self)
{
    ...
    snprintf(logpath, sizeof(logpath), "%s\\hmc_reduced_x87.log", g_dir);
    g_log = fopen(logpath, "w");
    logf_("HMC Reduced-Precision x87 loaded");
    ...
    if (fh != INVALID_HANDLE_VALUE) {
        do {
            ...
            snprintf(p->path, sizeof(p->path), "%s\\hmc_reduced_x87\\%s",
                     g_dir, fd.cFileName);
    ...
    if (!g_n_pending) {
        logf_("no patch files found in %s\\hmc_reduced_x87", g_dir);
        return;
    }
    ...
    if (reg && reg(0, on_dll_notify, NULL, &cookie) == 0)
        ...
        logf_("loader notifications unavailable");
    /* Poll as a backstop: HitmanContracts.exe is the main module and is
     * already mapped at process start (before this ASI's DllMain), so its
     * LDR "loaded" notification has already fired by the time we register —
     * the poller catches it on the first tick and applies the patch. (The
     * poll+gate design also transparently handles a packed module, which
     * becomes hookable only after its unpacker runs.) */
